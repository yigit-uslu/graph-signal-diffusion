#!/bin/bash
# Analyse wra_medium-large_outdoor_*_density channel scenarios sweeping "low, mid, high, ultra-low": generates networks, visualizes deployments,
...
CUDA_VISIBLE_DEVICES=0 python -m graph_signal_diffusion.cli.wra.analyze_channels \
    --config-name=channel_analysis/wra_medium-large_outdoor_low_density \
    "${OVERRIDE_ARGS[@]}" &
p1=$!
    # --cfg job --resolve

# CUDA_VISIBLE_DEVICES=1 python -m graph_signal_diffusion.cli.wra.analyze_channels \
#     --config-name=channel_analysis/wra_medium-large_outdoor_mid_density \
#     "${OVERRIDE_ARGS[@]}" &
# p2=$!
    # --cfg job --resolve
...
CUDA_VISIBLE_DEVICES=0 python -m graph_signal_diffusion.cli.wra.analyze_channels \
    --config-name=channel_analysis/wra_medium-large_outdoor_high_density \
    "${OVERRIDE_ARGS[@]}" &
p3=$!
    # --cfg job --resolve
...
# CUDA_VISIBLE_DEVICES=1 python -m graph_signal_diffusion.cli.wra.analyze_channels \
#     --config-name=channel_analysis/wra_medium-large_outdoor_ultra-low_density \
#     "${OVERRIDE_ARGS[@]}" &
# p4=$!
    # --cfg job --resolve
