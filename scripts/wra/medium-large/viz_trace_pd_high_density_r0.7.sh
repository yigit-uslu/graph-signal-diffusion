...
mid_k=$((2 * $mid_pairs))
bot_k=$((2 * $bot_pairs))
n_seeds=10          # number of figures to generate per r_min (one per seed)
base_seed=42
...
while IFS='|' read -r run_id run_timestamp; do
    ...
    for i in $(seq 0 $((n_seeds - 1))); do
        ...
        python -m graph_signal_diffusion.cli.wra.visualize_trace \
            ...
            '++receiver_selection.groups=[{quantile:[0.98,1.00],count:'"${top_k}"',palette:["#dc143c","#8b5cf6"],scatter_pairs:'"${top_pairs}"'},{quantile:[0.90,0.98],count:'"${mid_k}"',palette:["#f59e0b","#10b981"],scatter_pairs:'"${mid_pairs}"'},{quantile:[0.0,0.90],count:'"${bot_k}"',palette:["#3b82f6","#14b8a6"],scatter_pairs:'"${bot_pairs}"'}]' \
