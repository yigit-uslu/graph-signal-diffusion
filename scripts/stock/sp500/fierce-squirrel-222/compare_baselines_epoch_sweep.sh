#!/bin/bash
# Sequential EPOCH-SWEEP variant of compare_baselines.sh for fierce-squirrel-222.
#
...
#   These exist every 50 epochs (50,100,...,2000), so 100,200,...,900 are all
#   present. (best_models/ only has the sparse tracker-selected epochs
#   150/350/450/600/700, so it cannot give a clean every-100 sweep.) The
...
#   so RevIN alpha=1.1071, base_channels=128, num_layers=3, num_bottleneck_layers=3,
#   cond gated/attention flags, exploration kwargs, etc. auto-load correctly for
#   EVERY epoch — no per-epoch overrides.
#
#   NOTE on epoch range: this run went to 2000 epochs but PLATEAUED at ep 450
#   (composite rank-1) and OVERFIT the entire back half (val_loss 0.300 -> 0.312,
#   gap +0.009 -> +0.046 by ep 2000). Nothing past ep 450 beat it. The default
#   100..1000 sweep therefore covers the plateau (ep 450), the directional/
#   calibration pick (ep 700), and the early region; it skips the overfit tail
#   (ep 1000-2000, never saved as best_models). Extend `epochs` below to 2000 if
#   you specifically want to confirm the overfit/low-kurtosis tail on test.
#
# ============================ SPLIT WARNING ============================
# fierce-squirrel-222 was trained with dataset.n_split_chunks=10. This sweep
...
# sociable-frigatebird-619 (same dataset + 10-chunk split => identical test
# windows; this is the full-vs-small CAPACITY comparison). Do NOT set
# dataset.n_split_chunks=3 to "compare" against the 3-chunk family — that leaks
# ~80% of those test windows into this model's TRAIN set. See compare_baselines.sh
# in this directory for the full rationale.
# =======================================================================
#
...
#
# Runtime: at n_samples=20 + 100 DDIM steps on the FULL 128ch denoiser, each
# epoch is ~1-2h (test_subsample_n=50). A 10-epoch sweep (100..1000) is therefore
# ~10-20h, sequential on one GPU.
#
# Usage:
...
# epochs=(100 200 300 400 500 600 700 800 900 1000)   # which DDIM_epoch_${e}.pt to evaluate
epochs=(1200 1400 1600 1800 2000)                     # extend to the overfit tail if you want, but it won't beat ep 450
...
echo "=================================================================="
echo "fierce-squirrel-222 epoch sweep"
echo "  epochs:    ${epochs[*]}"
echo "  gpu:       $gpu"
