"""Trainer-level DDPM sampling diagnostics heatmap test.

This test runs real DDPM sampling with a UGNN model that has learned downsampling
at 3 selector levels (gamma=[1,2,2,2] -> skip first gamma=1 level), requests
sampling-phase selector probes at explicit timesteps, and verifies heatmap output.
"""
...
def _run_ddpm_sampling_selector_heatmap(
    output_dir: Path,
    *,
    patch_tqdm: bool = True,
) -> Path:
    """Run DDPM sampling diagnostics and return created heatmap path."""
    torch.manual_seed(0)
    ...
    tqdm_ctx = (
        patch("graph_signal_diffusion.diffusion.ddpm.tqdm.tqdm", lambda it, *args, **kwargs: it)
        if patch_tqdm
        else nullcontext()
    )
    ...
    with tqdm_ctx:
        ...
        assert sorted(selector_sampling_diagnostics.get("probe_timesteps", []), reverse=True) == sorted(
            set(probe_steps), reverse=True
        )
        ...
        assert len(probe_graph_count) == len(probe_steps)
...
def test_ddpm_sampling_selector_probe_heatmap_ugnn_gamma1222(tmp_path: Path) -> None:
    heatmap_path = _run_ddpm_sampling_selector_heatmap(
        output_dir=tmp_path / "selector_diagnostics_ddpm_sampling",
        patch_tqdm=True,
    )
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run DDPM sampling selector-heatmap diagnostics test workflow."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("tests/figs/selector_diagnostics_ddpm_sampling"),
...
if __name__ == "__main__":
    args = _parse_args()
    out = _run_ddpm_sampling_selector_heatmap(
