...
#
# Historically this module mutated ``matplotlib.rcParams`` at import time
# (huge label fonts, custom mathtext, seaborn darkgrid, ...).  Those
# mutations leaked into every pipeline that transitively imported this
# module, polluting unrelated plots.  Phase 2 of the plot-style cleanup
# moved the mutations into the dict below; callers that actually want
# the paper-figure look wrap their drawing in
# ``with matplotlib.rc_context(PAPER_FIGURE_RC_PARAMS):``.
# ---------------------------------------------------------------------------
_W = 469 / 72  # figure width in inches, approximately text width
_LABEL_SCALE = 2.25
_TICK_SCALE = 2.25
_LEGEND_SCALE = 2.0
...
def draw_stocks_graph_by_correlation(fundamentals_corr_graph, save_path):
    # Apply the paper-figure rcParams locally so this function's look
    # does not depend on global mutations (see PAPER_FIGURE_RC_PARAMS
